...
test('passes when a baseline is lowered', () => {
  withTempDir((dir) => {
    const base = writeBaseline(dir, 'base.json', { root: 10 });
    const head = writeBaseline(dir, 'head.json', { root: 6 });
    const { status } = run([base, head]);
    assert.equal(status, 0);
  });
});
...
test('fails when a baseline is raised', () => {
  withTempDir((dir) => {
    const base = writeBaseline(dir, 'base.json', { root: 10 });
    const head = writeBaseline(dir, 'head.json', { root: 20 });
...
test('fails when any of multiple workspaces is raised', () => {
  withTempDir((dir) => {
    const base = writeBaseline(dir, 'base.json', { root: 10, extra: 4 });
    const head = writeBaseline(dir, 'head.json', { root: 10, extra: 9 });
    const { status, stderr } = run([base, head]);
    assert.equal(status, 1);
    assert.match(stderr, /extra: baseline increased 4 → 9/);
  });
});
