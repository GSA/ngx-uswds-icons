...
// Dedicated config for the runtime WCAG 2.1 AA gate (GH #124). It reuses the
...
export default defineConfig({
  ...
  webServer: {
    // In CI the build/test job has already produced `dist/storybook/ngx-uswds-icons-demo`
    // (see .github/workflows/ci.yml), so reuse it and only serve. Locally there is
    ...
    command: process.env.CI
      ? 'npx --no-install http-server dist/storybook/ngx-uswds-icons-demo --port 6007 --silent'
      : 'npm run build-storybook -- --quiet && npx --no-install http-server dist/storybook/ngx-uswds-icons-demo --port 6007 --silent',
